...
//			Thread.yield();
	...
	for (QVal handle : map.keySet()) {
		if (!(map.get( handle ).equals( QMapSto.string4QVal( handle ) ))) {
			ok = false;
			System.out
				.println( "--- " + (QVal.asQVal( handle ) & 0xfffff) + ' ' + map.get( handle ) + " \t" + QMapSto.string4QVal( handle ) );
		}
		if (print) {
			System.out
				.println( "+++ " + (QVal.asQVal( handle ) & 0xfffff) + ' ' + map.get( handle ) + " \t" + QMapSto.string4QVal( handle ) );
...
protected boolean test_UtilMisc_createId() throws IOException {
	...
	ok = ok && (cmp0.substring( cmp0.length() - 4, cmp0.length() - 1 ).equals( cmp1.substring( cmp1.length() - 4, cmp1.length() - 1 ) ));
	System.out.println( "" + ok + " a a \t" + cmp0 + '\t' + cmp1 );
	cmp0 = cmp1;
	cmp1 = cmpx;
	cmpx = MiscFunc.createId( "a" );
	ok = ok && (cmp0.substring( cmp0.length() - 4, cmp0.length() - 1 ).equals( cmpx.substring( cmpx.length() - 4, cmpx.length() - 1 ) ));
	ok = ok && (cmp0.compareTo( cmp1 ) < 0);
	ok = ok && (cmp0.length() == cmp1.length());
	ok = ok && !(cmp0.substring( cmp0.length() - 4, cmp0.length() - 1 ).equals( cmp1.substring( cmp1.length() - 4, cmp1.length() - 1 ) ));
	...
	ok = ok && (cmp1.substring( cmp1.length() - 4, cmp1.length() - 1 ).equals( cmpx.substring( cmpx.length() - 4, cmpx.length() - 1 ) ));
	ok = ok && (cmp0.compareTo( cmp1 ) < 0);
	ok = ok && (cmp0.length() == cmp1.length());
	ok = ok && !(cmp0.substring( cmp0.length() - 4, cmp0.length() - 1 ).equals( cmp1.substring( cmp1.length() - 4, cmp1.length() - 1 ) ));
...
protected boolean test_QStr() {
	...
	ok = ok && (QStrFunc.shashBits4Double( "123", 0 )
			== QStrFunc.shashBits4shash( QStrFunc.shash( strX ), null ));
	if (!ok) {
		System.out.println( "Collation error!" );
	}
	String testS =
		"zyx *abc* 0 1 a+b a+1 1+a 1+2 a +b a+ b \n"
	...
	for (int i0 = 0; i0 < els.length; ++ i0) {
		...
		System.out.println( "" + el.replaceAll( "[^ -z]", "^" ) + "\t." + StringFunc.hexUtf16( sh )
			+ " \t" + (el.contains( "9" ) ? QStrFunc.date4ShashDate( sh )
				: (el.matches( "\\-?[0-9].*" ) ? QStrFunc.double4ShashNum( sh ) : QStrFunc.string4Shash( sh ))) );
...
protected boolean test_QStr_OLD( boolean print ) {
	String testS =
		"zyx *abc* 0 1   8 -1 -2 1996-12-08 0x20 22:22 5_cm 96-01-01\n- abc\n-abc\n"
			+ "#123 #045 ##123 28.02.99 28.03.1950 *def* g\n123 axx AxX aXX AXX";
	boolean ok = true;
	System.out.println( testS );
	String[] els = QStrFunc.markedAtoms4String( testS );
...
protected boolean test_UtilMisc_Time() {
	...
	System.out.println( "" + ok + '\t' + ts + ' ' + tx + ' ' + tim );
	tx = "20170102";
	tim = MiscFunc.slotSecond16oDate( tx );
	cmp = (2016L * 366 * 24 * 3600) << 16;
	ok = ok && (cmp < tim) && (tim < (cmp + 2 * ((24L * 3600) << 16)));
	System.out.print( "" + ok + '\t' + tx + ' ' + tim );
	tx = MiscFunc.date4SlotSecond16( tim );
	ok = ok && tx.startsWith( "2017-01" );
	...
	for (int round = 999; round >= 0; -- round) {
		...
		for (String atom : atoms) {
			...
			if ((QStrFunc.SHASH_OFFS <= atom.charAt( 0 )) && (QStrFunc.SHASH_TEMP_BITLIST != atom.charAt( 0 ))) {
				...
				if ((sh0 != atom.charAt( 0 )) && ((sh0 >= QStrFunc.SHASH_LITERAL) != (atom.charAt( 0 ) >= QStrFunc.SHASH_LITERAL))) {
...
protected boolean test_QMap_threaded( boolean print ) {
	...
	if (print) {
		System.out.println( "<<< (offset) "
			+ (QVal.asQVal( QMapSto.qval4AtomicLiteral( pooled, "A) long string for QMap " ) ) & ((1 << QMapSto.COMBINED_BITS__28) - 1)) );
	}
	...
	if (print) {
		System.out.println( "<<< (35*2+2+offset) "
			+ (QVal.asQVal( QMapSto.qval4AtomicLiteral( pooled, "B) long string for QMap" ) ) & ((1 << QMapSto.COMBINED_BITS__28) - 1)) );
	}
	return ok;
}
...
//			ok = ok && rfcCheck( in, out, old );
	...
	ok = ok && rfcCheck( "__x_ abc{_{def__gab_{c}}g_{{def__gabc}}g", "_xabc\u0002def_gab\u000ec\u000f}g\u0002def__gabc\u0003g", true );
	ok = ok && rfcCheck( "__x_ abc_{{def__gab_{c}}_g_{{def__gabc}}g", "_xabc\u0002def_gab{c\u0003g{{def_gabc}}g", false );
